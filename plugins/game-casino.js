...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
  ...
  cooldowns[m.sender] = Date.now()
        ...
        if (args.length < 1) return conn.reply(m.chat, '🚩 Ingresa la cantidad de ' + `*❇️ Eris*` + ' que deseas aportar contra' + ` *Naruto Ai*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)

        if (user.limit >= count * 1) {
            user.limit -= count * 1
            if (Aku > Kamu) {
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Naruto Ai* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} ❇️ Eris.`.trim(), m, rcanal)
            } else if (Aku < Kamu) {
                user.limit += count * 2
