...
let handler = async (m, {conn, usedPrefix}) => {
    ...
    let img = await (await fetch(`https://i.ibb.co/nz8hqJz/Sylph.jpg`)).buffer()
    let txt = ` –  *S T A R S  -  U S E R*\n\n`
        txt += `┌  ✩  *Nombre* : ${user.name}\n`
        txt += `│  ✩  *Eris* : ${toNum(user.limit)} ( *${user.limit}* )\n`
...
handler.help = ['eris']
handler.tags = ['rpg']
handler.command = ['coins', 'wallet', 'cartera', 'eris', 'eri', 'bal', 'balance']
